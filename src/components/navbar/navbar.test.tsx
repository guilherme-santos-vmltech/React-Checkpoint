...
return function MockWishlistModal({ isOpen }: { isOpen: boolean; onClose: () => void }) {
...
return function MockCartModal({ isOpen }: { isOpen: boolean; onClose: () => void }) {
  if (!isOpen) return null;
  return (
...
const createMockCartContext = (cartItems: CartItem[] = []): CartContextType => ({
cartItems,
...
const mockUseCart = jest.fn<CartContextType, []>(() => createMockCartContext([]));
...
test('renders shopping action buttons', () => {
  renderNavbar();
  expect(screen.getByRole('button', { name: /open wishlist/i })).toBeInTheDocument();
...
test('opens wishlist modal when clicking wishlist button', () => {
  ...
  expect(screen.getByRole('dialog', { name: 'Wishlist' })).toBeInTheDocument();
});
...
test('handles empty cart state correctly', () => {
  renderNavbar();
  const cartButton = screen.getByRole('button', { name: /open cart with 0 items/i });
  expect(cartButton).toBeInTheDocument();
